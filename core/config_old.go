...
func CreateOneDriveFromConfigFile() (od OldOneDrive, err error) {
	argNum := len(os.Args)
	...
	if err != nil {
		return OldOneDrive{}, err
	}
	return od, nil
}

func (od *OldOneDrive) SaveConfigFile() error {
	configFile := "config.json"
...
func (od *OldOneDrive) HotReloadConfigFile() error {
	argNum := len(os.Args)
	configFile := "config.json"
	if argNum > 1 {
...
func (od *OldOneDrive) SaveDriveCacheFile() error {
...
func (od *OldOneDrive) LoadDriveCacheFile() error {
	cacheFile := "cache.json"
